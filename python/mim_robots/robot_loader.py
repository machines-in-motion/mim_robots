...
def load_bullet_wrapper(robot_name, locked_joints=None):
    ...
    try:
        if robot_name == "iiwa":
            ...
            from mim_robots.robots.teststand.pinbullet.teststand_wrapper import (
                TeststandRobot,
            )
            ...
            from mim_robots.robots.panda.pinbullet.pandaWrapper import PandaRobot
            return PandaRobot(MiM_Robots["panda"], locked_joints)
        else:
            assert False
    except:
        try:
    ...
    try:
        ...
        try:
            if RobotInfo.fixed_base:
                ...
                full_robot = pin.RobotWrapper.BuildFromURDF(
                    RobotInfo.urdf_path, RobotInfo.mesh_dir, pin.JointModelFreeFlyer()
                )
            if locked_joints is None:
                ...
                reduced_model, [visual_model, collision_model] = pin.buildReducedModel(
                    full_robot.model,
                    [full_robot.visual_model, full_robot.collision_model],
                    locked_joints_ids,
                    np.zeros(RobotInfo.nq),
                )
                return pin.robot_wrapper.RobotWrapper(
                    reduced_model, collision_model, visual_model
                )
        except Exception as e:
